...
You can also run migrations and seeding manually after logging in to the web UI by going to the generated backend host port (`BACKEND_PORT`) from `.dev-ports.env`:

- `http://localhost:<BACKEND_PORT>/migrate/latest`
- `http://localhost:<BACKEND_PORT>/migrate/up`
- `http://localhost:<BACKEND_PORT>/migrate/down`
- `http://localhost:<BACKEND_PORT>/migrate/seed`
...
by default.
Configure Auth0 once with `http://*.traditional-knowledge.localhost/callback` in Allowed Callback URLs, and `http://*.traditional-knowledge.localhost` in Allowed Logout URLs and Allowed Web Origins. The wildcard applies to the gateway's browser-facing hostname derived from `WEB_PORT`; browser traffic enters through the shared gateway. This hostname suffix resolves only to the local machine; no public domain or DNS service is used.
