...
import { CreateService } from "@/services/information-sharing-agreements/generate-confidentiality-receipt"

import BaseController from "@/controllers/base-controller"

export class GenerateConfidentialityReceiptController extends BaseController<InformationSharingAgreement> {
  async create() {
    try {
      ...
      if (!policy.show()) {
        return this.response.status(403).json({
          message:
            "You are not authorized to generate a confidentiality receipt for this information sharing agreement.",
        })
      }
      ...
      logger.error(`Failed to generate confidentiality receipt: ${error}`, { error })
      ...
      return this.response.status(400).json({
        message: `Failed to generate confidentiality receipt: ${error}`,
      })
    }
  }
  ...
  private buildFileName(informationSharingAgreement: InformationSharingAgreement) {
    ...
    return `Confidentiality Receipt, ${displayName}, ${currentDateTime}.docx`
  }
...
export default GenerateConfidentialityReceiptController
